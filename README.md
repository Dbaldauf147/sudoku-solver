...
- **`index.html`** — a zero-dependency static front end that houses all the
  ...
  - Every time you **import a screenshot** the puzzle is **saved to your
    ...
    **delete** any puzzle.
  - Nearly every action lives under a ⚙ menu in the top-right corner: the
    profile chip ("Playing as: …"), Home, Game library and Import screenshot
    lead it, then Start solving, Hint, Step-by-step solve, Stats, Techniques,
    Check entries, Settings, Reveal solution, Edit puzzle and the rest. Only
    **Practice** stays out in the top bar, so the board gets the room.
