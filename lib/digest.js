...
function analyze(games) {
  // Games put down part-way are recorded too, but they don't belong in a like-for-like comparison:
  // a puzzle abandoned five moves in has no solve time to compare, and counting it as a game would
  // quietly flatter every per-game rate below (mistakes, hints) by averaging over a stub.
  const all = chron((games || []).filter((g) => g && g.moves && !g.unfinished));
